...
import { apiGetHoldings, apiGetProjects, apiGetWallet, type WalletData } from "@/lib/api-client";
import type { Holding, ProjectStock } from "@/lib/data";
...
export default function PortfolioPage() {
  ...
  useEffect(() => {
    Promise.all([apiGetHoldings("demo"), apiGetProjects(), apiGetWallet("demo")])
      .then(([h, p, w]) => {
        setHoldings(h);
        setProjects(p);
        setWallet(w);
  ...
  const summaryCards = useMemo(() => ([
    { label: "Total Invested", value: wallet?.invested ?? 0, color: "text-foreground", pnl: false },
    { label: "Current Value", value: wallet?.currentValue ?? 0, color: "text-foreground", pnl: false },
    { label: "Total P&L", value: wallet?.pnl ?? 0, color: "text-gain", pnl: true },
  ]), [wallet]);
