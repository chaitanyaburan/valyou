...
type RawProject = Partial<StartupProject> & {
  title?: string;
  tagline?: string;
  price?: number;
};

function normalizeProject(project: RawProject, index: number): StartupProject {
  const totalTokens = typeof project.total_tokens === "number" && project.total_tokens > 0
    ? project.total_tokens
    : 100_000;
  const soldTokens = typeof project.sold_tokens === "number" && project.sold_tokens >= 0
    ? Math.min(project.sold_tokens, totalTokens)
    : 0;
  const pricePerToken = typeof project.price_per_token === "number"
...
function PortfolioDashboard() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      const data = (await response.json()) as { projects?: RawProject[] } | RawProject[];
  ...
  const investmentRows = useMemo(() => {
    return investments.map((investment) => {
      ...
      const utilization =
        project && project.total_tokens > 0 ? project.sold_tokens / project.total_tokens : 0;
